            ...
            try {   
                ...
                try {
                    ...
                    while(rs.next()){
                        ...
                        String kdpjlab = Sequel.cariIsi("select kd_dokterlab from set_pjlab");
                        String nmpjlab = Sequel.cariIsi("select nm_dokter from dokter where kd_dokter = ?", kdpjlab);
                        param.put("nmpjlab", nmpjlab);
                        finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",kdpjlab);
                        param.put("finger","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+nmpjlab+"\nID "+(finger.equals("")?kdpjlab:finger)+"\n"+rs.getString("tgl_periksa"));  
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                if(akses.gethapus_berkas_digital_perawatan()==true){
                    berkas.loadURL("https://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/"+"berkasrawat/login2.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rawat="+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString());
                }else{
                    berkas.loadURL("https://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/"+"berkasrawat/login2nonhapus.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rawat="+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString());
                }   
            } catch (Exception ex) {
