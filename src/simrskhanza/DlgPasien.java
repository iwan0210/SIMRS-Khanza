...
public class DlgPasien extends javax.swing.JDialog {
    ...
    public DlgPasien(java.awt.Frame parent, boolean modal) {
        ...
        PekerjaanPj.setDocument(new batasInput((byte)35).getKata(PekerjaanPj));
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNo.getText().trim().equals("")){
            ...
            if (TNm.getText().trim().toLowerCase().startsWith("by ny")) {
                String nikBayi = noKTP.replace("-", "");

                if (nikBayi.isBlank()) {
                    JOptionPane.showMessageDialog(null,
                        "Maaf, NIK bayi tidak boleh kosong, bisa pakai NIK ibu!!!");
                    return;
                }
            } else {
                if (!noKTP.contains("-")) {
                    int cariPasien = Sequel.cariInteger(
                        "select count(no_rkm_medis) from pasien where no_ktp = ?",
                        TKtp.getText().trim()
                    );

                    if (cariPasien > 0) {
                        JOptionPane.showMessageDialog(null,
                            "Maaf, data pasien dengan NIK tersebut sudah ada!!!");
                        return;
                    }
                }
            }
...
private void KabupatenMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_KabupatenMouseMoved
    ...
    private void BtnSimpan6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpan6ActionPerformed
        if(NoRmTujuan.getText().trim().equals("")){
            ...
            Sequel.mengedit("pasien_meninggal","no_rkm_medis=?","no_rkm_medis=?",2,new String[]{
                NoRmTujuan.getText(),TNo.getText()
            });
                ...
                try {
                    ...
                    if(rs.next()){
                        if(rs.getString("gambar").equals("")||rs.getString("gambar").equals("-")){
                            ...
                            LoadHTML.setText("<html><body><center><img src='https://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/photopasien/"+rs.getString("gambar")+"' alt='photo' width='200' height='200'/></center></body></html>");
                        }  
