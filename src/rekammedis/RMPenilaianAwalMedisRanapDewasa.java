...
public final class RMPenilaianAwalMedisRanapDewasa extends javax.swing.JDialog {
    ...
    public RMPenilaianAwalMedisRanapDewasa(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Penunjang Lainnya", "Diagnosis/Asesmen", "Tatalaksana", "Edukasi", "Rencana inap", "Keterangan inap", "Perawatan Lanjutan"
        }) {
            @Override
    ...
    private void initComponents() {
        ...
        FormInput.setPreferredSize(new java.awt.Dimension(1417, 1383));
        ...
        Hubungan.setBounds(780, 40, 80, 23);
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "07-08-2026 09:36:38" }));
        ...
        PerawatanLnjt.setBounds(980, 900, 240, 22);

        jLabel84.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel84.setText("Perawatan Lanjutan:");
        jLabel84.setName("jLabel84"); // NOI18N
        FormInput.add(jLabel84);
        jLabel84.setBounds(870, 900, 110, 23);

        jLabel85.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel85.setText("Rencana Dirawat:");
        jLabel85.setName("jLabel85"); // NOI18N
        FormInput.add(jLabel85);
        jLabel85.setBounds(870, 870, 90, 23);

        jLabel83.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel83.setText("Hari");
        jLabel83.setName("jLabel83"); // NOI18N
        FormInput.add(jLabel83);
        jLabel83.setBounds(1220, 870, 30, 23);

        jLabel105.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        jLabel105.setText("VII. DISCHARGE PLANNING");
        jLabel105.setName("jLabel105"); // NOI18N
        FormInput.add(jLabel105);
        jLabel105.setBounds(870, 840, 190, 23);
        ...
        rencana.setBounds(960, 870, 180, 22);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (TNoRM.getText().trim().equals("")) {
            ...
            if (Sequel.menyimpantf("penilaian_medis_ranap", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 43, new String[]{
                ...
                hari.getText(), PerawatanLnjt.getSelectedItem().toString(), rencana.getSelectedItem()
                .toString()}) == true) {
        ...
        try {
            if (TCari.getText().trim().equals("")) {
                ps = koneksi.prepareStatement(
                        ...
                        + "penilaian_medis_ranap.kepala,penilaian_medis_ranap.mata,penilaian_medis_ranap.gigi,penilaian_medis_ranap.tht,penilaian_medis_ranap.thoraks,penilaian_medis_ranap.jantung,penilaian_medis_ranap.paru,penilaian_medis_ranap.abdomen,penilaian_medis_ranap.ekstremitas,"
                        + "penilaian_medis_ranap.genital,penilaian_medis_ranap.kulit,penilaian_medis_ranap.ket_fisik,penilaian_medis_ranap.ket_lokalis,penilaian_medis_ranap.lab,penilaian_medis_ranap.rad,penilaian_medis_ranap.penunjang,penilaian_medis_ranap.diagnosis,penilaian_medis_ranap.tata,"
                        + "penilaian_medis_ranap.edukasi,penilaian_medis_ranap.hari_rawat,penilaian_medis_ranap.perawatan_lanjut,penilaian_medis_ranap.rencana,dokter.nm_dokter "
                        + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join penilaian_medis_ranap on reg_periksa.no_rawat=penilaian_medis_ranap.no_rawat "
                        + "inner join dokter on penilaian_medis_ranap.kd_dokter=dokter.kd_dokter where "
                        + "penilaian_medis_ranap.tanggal between ? and ? order by penilaian_medis_ranap.tanggal");
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("diagnosis"), rs.getString("tata"), rs.getString("edukasi"), rs.getString("rencana"),rs.getString("hari_rawat"), rs.getString("perawatan_lanjut"),
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            rencana.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(), 45).toString());
            hari.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 46).toString());
            PerawatanLnjt.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(), 47).toString());
    ...
    private void ganti() {
        if (Sequel.mengedittf("penilaian_medis_ranap", "no_rawat=?", "no_rawat=?,tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,keluhan_utama=?,rps=?,rpk=?,rpd=?,rpo=?,alergi=?,keadaan=?,gcs=?,kesadaran=?,td=?,nadi=?,rr=?,suhu=?,"
                + "spo=?,bb=?,tb=?,kepala=?,mata=?,gigi=?,tht=?,thoraks=?,jantung=?,paru=?,abdomen=?,genital=?,ekstremitas=?,kulit=?,ket_fisik=?,ket_lokalis=?,lab=?,rad=?,penunjang=?,diagnosis=?,tata=?,edukasi=?,hari_rawat=?,perawatan_lanjut=?,rencana=?", 44, new String[]{
                    TNoRw.getText(), Valid.SetTgl(TglAsuhan.getSelectedItem() + "") + " " + TglAsuhan.getSelectedItem().toString().substring(11, 19), KdDokter.getText(), Anamnesis.getSelectedItem().toString(), Hubungan.getText(),
                    KeluhanUtama.getText(), RPS.getText(), RPD.getText(), RPK.getText(), RPO.getText(), Alergi.getText(), Keadaan.getSelectedItem().toString(), GCS.getText(), Kesadaran.getSelectedItem().toString(), TD.getText(),
                    Nadi.getText(), RR.getText(), Suhu.getText(), SPO.getText(), BB.getText(), TB.getText(), Kepala.getSelectedItem().toString(), Mata.getSelectedItem().toString(), Gigi.getSelectedItem().toString(), THT.getSelectedItem().toString(),
                    Thoraks.getSelectedItem().toString(), Jantung.getSelectedItem().toString(), Paru.getSelectedItem().toString(), Abdomen.getSelectedItem().toString(), Genital.getSelectedItem().toString(), Ekstremitas.getSelectedItem().toString(),
                    Kulit.getSelectedItem().toString(), KetFisik.getText(), KetLokalis.getText(), Laborat.getText(), Radiologi.getText(), Penunjang.getText(), Diagnosis.getText(), Tatalaksana.getText(), Edukasi.getText(),
                    hari.getText(), PerawatanLnjt.getSelectedItem().toString(), rencana.getSelectedItem().toString(),
                    tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
                }) == true) {
            tampil();
            emptTeks();
